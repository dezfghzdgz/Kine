...
// Načtení TS bez překladače: ze souboru se odstraní typy. Je to hrubé, ale
// drží test na skutečném zdrojáku místo na kopii, která se rozejde.
const zdroj = readFileSync(new URL('../lib/playerStart.ts', import.meta.url), 'utf8')
  .replace(/^export type StartOptions = \{[\s\S]*?^\};$/m, '')
  .replace(/: StartOptions/g, '')
...
const modul = await import(
  'data:text/javascript;base64,' + Buffer.from(zdroj).toString('base64')
);
const { startPlayback, ATTEMPT_SCHEDULE, OFFER_TAP_AFTER_MS, SEEK_TOLERANCE_S } = modul;
